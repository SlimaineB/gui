/* tslint:disable max-line-length */
import { TestBed, getTestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { HttpClient, HttpResponse } from '@angular/common/http';
import { of } from 'rxjs';
import { take, map } from 'rxjs/operators';
import * as moment from 'moment';
import { DATE_TIME_FORMAT } from 'app/shared/constants/input.constants';
import { CmActionService } from 'app/entities/cm-action/cm-action.service';
import { ICmAction, CmAction } from 'app/shared/model/cm-action.model';

describe('Service Tests', () => {
    describe('CmAction Service', () => {
        let injector: TestBed;
        let service: CmActionService;
        let httpMock: HttpTestingController;
        let elemDefault: ICmAction;
        let currentDate: moment.Moment;
        beforeEach(() => {
            TestBed.configureTestingModule({
                imports: [HttpClientTestingModule]
            });
            injector = getTestBed();
            service = injector.get(CmActionService);
            httpMock = injector.get(HttpTestingController);
            currentDate = moment();

            elemDefault = new CmAction(0, 0, 0, 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', currentDate, 0);
        });

        describe('Service methods', async () => {
            it('should find an element', async () => {
                const returnedFromService = Object.assign(
                    {
                        actionDateTime: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                service
                    .find(123)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: elemDefault }));

                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should create a CmAction', async () => {
                const returnedFromService = Object.assign(
                    {
                        id: 0,
                        actionDateTime: currentDate.format(DATE_TIME_FORMAT)
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        actionDateTime: currentDate
                    },
                    returnedFromService
                );
                service
                    .create(new CmAction(null))
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'POST' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should update a CmAction', async () => {
                const returnedFromService = Object.assign(
                    {
                        actionId: 1,
                        actionNum: 1,
                        actionType: 'BBBBBB',
                        actionDescription: 'BBBBBB',
                        actionInput: 'BBBBBB',
                        actionOutput: 'BBBBBB',
                        actionDateTime: currentDate.format(DATE_TIME_FORMAT),
                        actionDuration: 1
                    },
                    elemDefault
                );

                const expected = Object.assign(
                    {
                        actionDateTime: currentDate
                    },
                    returnedFromService
                );
                service
                    .update(expected)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'PUT' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should return a list of CmAction', async () => {
                const returnedFromService = Object.assign(
                    {
                        actionId: 1,
                        actionNum: 1,
                        actionType: 'BBBBBB',
                        actionDescription: 'BBBBBB',
                        actionInput: 'BBBBBB',
                        actionOutput: 'BBBBBB',
                        actionDateTime: currentDate.format(DATE_TIME_FORMAT),
                        actionDuration: 1
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        actionDateTime: currentDate
                    },
                    returnedFromService
                );
                service
                    .query(expected)
                    .pipe(
                        take(1),
                        map(resp => resp.body)
                    )
                    .subscribe(body => expect(body).toContainEqual(expected));
                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify([returnedFromService]));
                httpMock.verify();
            });

            it('should delete a CmAction', async () => {
                const rxPromise = service.delete(123).subscribe(resp => expect(resp.ok));

                const req = httpMock.expectOne({ method: 'DELETE' });
                req.flush({ status: 200 });
            });
        });

        afterEach(() => {
            httpMock.verify();
        });
    });
});
